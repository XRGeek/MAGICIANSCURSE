...
public class bl_LoadingScreenUI : MonoBehaviour
{
    ...
    public void Init(bl_SceneLoader loader)
    {
        ...
        if (Manager.HasTips) 
        {
            ...
            {
                case "English":
                    cacheTips = Manager.TipListEnglish;
                    break;
                case "French":
                    cacheTips = Manager.TipListFrench;
                    break;
                case "German":
                    cacheTips = Manager.TipListGerman;
                    break;
            }  
        }
    ...
    public void ChangeTipsLanguage()
    {
        if (Manager.HasTips)
        {
            string Language = GleyLocalization.Manager.GetCurrentLanguage().ToString();
    ...
    public void SetupUIForScene(bl_SceneLoaderInfo info)
    {
        ...
        if (SceneNameText != null)
        {
            ...
            {
                case "English":
                    SceneNameText.text = info.DisplayNameEnglish;
                    break;
                case "French":
                    SceneNameText.text = info.DisplayNameFrench;
                    break;
                case "German":
                    SceneNameText.text = info.DisplayNameGerman;
                    break;
            }        
        }
    ...
    {
        FadeImageCanvas.blocksRaycasts = true;
        FadeImageCanvas.alpha = 0;



        if (LoadingCircleCanvas != null) { StartCoroutine(FadeInCanvas(LoadingCircleCanvas, 0.5f)); }
        if (LoadingBarAlpha != null) { StartCoroutine(FadeInCanvas(LoadingBarAlpha, 1)); }
        else { LoadBarSlider?.gameObject?.SetActive(true); FilledImage?.gameObject?.SetActive(true); }
        ...
        FadeImageCanvas.alpha = 0;
        FadeImageCanvas.blocksRaycasts = false;
    }
